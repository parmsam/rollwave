...
- **Timer ring**: `TimerDisplay.tsx`'s stroke width scales continuously with `progressRatio` (8px → 26px over the whole phase), not just during the warning window — a "time visibly passing" cue meant to be noticeable throughout, not just in the final seconds. The red glow/pulse (`isWarning`) is a separate, additional layer on top of this, only in the last `warningSeconds`.
- Header icons (`FullscreenToggle`, `GitHubLink`, `VolumeToggle`, history toggle, optional `LiveClock`) live inline in `app.tsx`'s header row — `FullscreenToggle` is feature-detected (`useFullscreen.ts`) and only renders when the Fullscreen API is available.
